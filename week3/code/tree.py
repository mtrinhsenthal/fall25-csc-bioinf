...
class TreeNode:
    def __init__(self, children=None, distances=None, index=None):
        ...
        if index is None:
            if children is None or distances is None:
                raise TypeError("Either reference index or child nodes must be set")
    ...
    def distance_to(self, node, topological=False):
        ...
        def distance_up(n):
            dist = 0.0
            while n is not lca:
                dist += 1 if topological else n._distance
                n = n._parent
            return dist

        return distance_up(self) + distance_up(node)

    def lowest_common_ancestor(self, node):
        path_self = self._path_to_root()
        path_other = node._path_to_root()
        lca = None
        for a, b in zip(reversed(path_self), reversed(path_other)):
            if a is b:
                lca = a
            else:
                break
    ...
    def from_newick(newick, labels=None):
        ...
        if not newick:
            raise InvalidFileError("Empty Newick string")

        if "(" not in newick:
            parts = newick.split(":")
            label = parts[0]
            distance = float(parts[1]) if len(parts) > 1 else 0
        ...
        for ch in inner:
            if ch == "(":
                ...
                if current:
                    child, _ = TreeNode.from_newick(current, labels)
        ...
        remainder = newick[stop + 1 :]
        distance = 0.0
        if ":" in remainder:
            _, dist_str = remainder.split(":")
            distance = float(dist_str)
        node = TreeNode(children, [0.0] * len(children))
